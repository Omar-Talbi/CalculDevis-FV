...
import HorizontalScrollingMenu from "./HorizontalScrollingMenu/HorizontalScrollingMenu";
...
function ParametresProfiler() {
  ...
  return (
    <div className="parametres">
      ...
      <div className="parametres_Table">
        <span className="items_parametres_Table">
          {article.map((data) => {
            return (
              <span key={data._id}>
                <Button
          ...
          {/* Horizontal Scrolling Menu component call*/}
      ...
      <div className="parametres_Table">
        <span className="items_parametres_Table">
          {sousArticleItem.map((data) => {
            return (
              <span key={data._id} className="paramatersButtons">
                <Button
                  onClick={() => afficherSerie(data.Nom, data._id)}
                  className="paramétres_profiler"
                  size="large"
                  variant="contained"
                  color="success"
                ...
                <BtnRenderParamsProfiler
                  data={data}
                  deleteSousArticle={deleteSousArticle}
                />
              </span>
            );
          })}
      ...
      <div>
        {attributItem.map((data) => {
          return (
            <span key={data._id} className="paramatersButtons">
              ...
              <span className="td_table_next" key={data._id}>
                {data.Nom}
              </span>
              ...
              {profilerItem.map((item) => {
                if (data._id === item.attribut)
                  return (
                    <span key={item._id} className="td_table_next">
                      {item.reference}
                    </span>
                  );
              })}
        ...
        <span className="paramatersButtons">
          <span>
            {verItem.map((data) => {
              return (
                <span key={data._id}>
      ...
      <Modal
        ...
        aria-describedby="child-modal-description"
      >
        <ModalAjoutSousArticle
          articleID={articleID}
          handleCloseSousArticle={handleCloseSousArticle}
        />
      </Modal>
      <Modal
        hideBackdrop
      ...
      >
        <ModalAjoutAttribut
          serieID={serieID}
          handleCloseAttribut={handleCloseAttribut}
        />
      </Modal>
      <Modal
        hideBackdrop
      ...
      >
        <ModalAjoutProfiler
          ...
          handleCloseProfiler={handleCloseProfiler}
        />
      </Modal>
      <Modal
        hideBackdrop
